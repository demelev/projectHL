...
internal abstract class NativeCompiler : INativeCompiler
{
    ...
    {
        <Aggregate>c__AnonStorey62 storey = new <Aggregate>c__AnonStorey62 {
            prefix = prefix,
            suffix = suffix
        };
        return items.Aggregate<string, string>(string.Empty, new Func<string, string, string>(storey.<>m__CF));
    }
    ...
    {
        <ParallelFor>c__AnonStorey63<T> storey = new <ParallelFor>c__AnonStorey63<T> {
            sources = sources,
            action = action
        };
        ...
        {
            threadArray[i] = new Thread(new ParameterizedThreadStart(storey.<>m__D0));
        }
    ...
    [CompilerGenerated]
    private sealed class <Aggregate>c__AnonStorey62
    {
        internal string prefix;
        internal string suffix;

        internal string <>m__CF(string current, string additionalFile)
        {
    ...
    [CompilerGenerated]
    private sealed class <ParallelFor>c__AnonStorey63<T>
    {
        internal Action<T> action;
        internal T[] sources;

        internal void <>m__D0(object obj)
        {
            int num;
