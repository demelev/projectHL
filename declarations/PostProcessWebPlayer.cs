...
internal class PostProcessWebPlayer
{
    ...
    {
        <GeneratePlayerParamsString>c__AnonStorey61 storey = new <GeneratePlayerParamsString>c__AnonStorey61 {
            options = options
        };
        return string.Format("{{ {0} }}", string.Join(",", optionNames.Select<KeyValuePair<BuildOptions, string>, string>(new Func<KeyValuePair<BuildOptions, string>, string>(storey.<>m__CB)).ToArray<string>()));
    }
    ...
    [CompilerGenerated]
    private sealed class <GeneratePlayerParamsString>c__AnonStorey61
    {
        internal BuildOptions options;

        internal string <>m__CB(KeyValuePair<BuildOptions, string> pair)
        {
