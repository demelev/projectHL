namespace UnityEditorInternal
{
    ...
    internal class AnimationWindowHierarchyGUI : TreeViewGUI
    {
        ...
        private GenericMenu GenerateMenu(List<AnimationWindowHierarchyNode> interactedNodes)
        {
            ...
            if (flag2)
            {
                menu.AddItem(new GUIContent("Interpolation/Euler Angles"), rotationInterpolationMode == RotationCurveInterpolation.Mode.Baked, new GenericMenu.MenuFunction2(this.ChangeRotationInterpolation), RotationCurveInterpolation.Mode.Baked);
        ...
        public override Rect OnRowGUI(TreeViewItem node, int row, float rowWidth, bool selected, bool focused)
        {
            ...
            if (!node2.topPixel.HasValue)
            {
                node2.topPixel = new float?(this.GetTopPixelOfRow(row, base.m_TreeView.data.GetVisibleRows()));
            }
            float nodeHeight = this.GetNodeHeight(node2);
            Rect rect = new Rect(0f, node2.topPixel.Value, rowWidth, nodeHeight);
            this.DoNodeGUI(rect, node2, selected, focused, row);
            return rect;
        }
